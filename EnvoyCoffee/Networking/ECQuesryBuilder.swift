...
class ECQuesryBuilder: NSObject {
    ...
    class func getEnvoyCoffeeShopsCompleteQuery() -> String{
        return ECQuesryBuilder.foursquareSearchURL() + "?" + ECQuesryBuilder.authorizedQuery() + "&ll=37.775176,-122.397800" + "&limit=15" + "&query=coffee"
    }
    
    class func detailsQuery(_ id:String) -> String{
        return String.init(format: "%@%@?", foursquareBaseURL,id) + ECQuesryBuilder.authorizedQuery()
    }
}
